...
export default function QBPassMap({ playerId, season }: QBPassMapProps) {
  ...
  useEffect(() => {
    async function fetchPassMapData() {
      ...
      if (season < 2010) {
        setError('Pass map data is only available from 2010 onwards. Please select a more recent season.');
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (errorData.isPre2010) {
            throw new Error('Pass map data is only available from 2010 onwards. Please select a more recent season.');
          }
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
        <div className="text-sm text-blue-900">
          ...
          <ul className="list-disc list-inside space-y-1 text-xs">
            <li>QB pass maps are only available from 2010 onwards.</li>
